...
function seed(db) {
  if (db.prepare('SELECT COUNT(*) AS count FROM services').get().count === 0) {
    ...
    const rows = [
      ['manicure', 'Маникюр', 'Аккуратная обработка кутикулы, аппаратная или классическая техника, покрытие гель-лаком с укреплением и стойким результатом на три недели.', '90 мин', 'от 800 ₽', 'services', 1, 0, '/images/catalog-manicure.jpg'],
      ['pedicure', 'Педикюр', 'Полная обработка стоп, удаление мозолей и натоптышей, уход за кожей, аккуратное покрытие гель-лаком на длительный срок.', '120 мин', 'от 1 700 ₽', 'services', 1, 1, '/images/catalog-pedicure.jpg'],
      ['brows', 'Брови', 'Коррекция формы под особенности лица, стойкое окрашивание хной или краской, ламинирование для естественного ухоженного взгляда.', '60 мин', 'от 500 ₽', 'services', 1, 2, '/images/catalog-brows.jpg'],
      ['lashes', 'Ресницы', 'Наращивание классическое или объёмное, деликатное ламинирование, окрашивание — подчеркнём взгляд без ежедневного макияжа.', '90 мин', 'от 2 000 ₽', 'services', 1, 3, '/images/catalog-lashes.jpg'],
      ['makeup', 'Макияж', 'Дневной, вечерний, свадебный или фото-макияж с учётом типа кожи и пожеланий — стойкий результат на весь день.', '60 мин', 'от 3 000 ₽', 'services', 1, 4, '/images/catalog-makeup.jpg'],
      ['hairstyles', 'Прически', 'Укладки на каждый день, вечерние и свадебные прически любой сложности с использованием профессиональной косметики.', '60 мин', 'от 3 500 ₽', 'services', 1, 5, '/images/catalog-hairstyles.jpg'],
      ['lamination', 'Ламинирование', 'Ламинирование бровей и ресниц для естественного объёма и формы без наращивания — эффект держится до месяца.', '60 мин', 'от 1 500 ₽', 'services', 0, 6, '/images/catalog-lamination.jpg'],
      ['kids', 'Детские услуги', 'Маникюр, причёски и укладки для юных клиенток в комфортной атмосфере, бережно и с заботой о ребёнке.', '45 мин', 'от 500 ₽', 'services', 0, 7, '/images/catalog-kids.jpg'],
      ['adults', 'Взрослые услуги', 'Полный комплекс основных процедур для взрослых клиентов — от маникюра до макияжа в одной студии.', '60–120 мин', 'от 700 ₽', 'services', 0, 8, '/images/catalog-adults.jpg'],
  ...
  if (db.prepare('SELECT COUNT(*) AS count FROM price_items').get().count === 0) {
    ...
    const insert = db.prepare('INSERT INTO price_items (group_slug, name, description, duration, price, is_extra, sort_order) VALUES (?, ?, ?, ?, ?, ?, ?)');
    const transaction = db.transaction(() => {
      priceSource.groups.forEach((group) => {
        ...
        [...group.items, ...group.extras].forEach((item, index) => {
          ...
          insert.run(group.slug, item.name, item.description || '', item.duration || '', item.price, isExtra, order);
          order += 1;
        });
      });
    });
