...
const labelFor = (item, section) => section === 'reviews' ? item.author : section === 'promotions' ? item.title : item.name;
  ...
  return <main className={styles.dashboard}>
    ...
    <div className={styles.shell}>
      ...
      <section className={styles.content}>
        <div className={styles.pageTitle}><div><p className={styles.eyebrow}>УПРАВЛЕНИЕ САЙТОМ</p><h1>{settings.label}</h1><p>{settings.description}</p></div><button className={styles.primaryButton} onClick={createItem}>+ Добавить</button></div>
        ...
        <div className={styles.list} aria-busy={loading}>{loading ? <p>Загрузка…</p> : items.length === 0 ? <p className={styles.empty}>Здесь пока ничего нет. Добавьте первую карточку.</p> : items.map((item, index) => <article className={styles.item} key={item.id}>
          ...
          <div className={styles.itemText}><h2>{labelFor(item, section)}</h2><p>{section === 'reviews' ? item.text : section === 'promotions' ? `${item.discount} · ${item.expiry}` : `${item.price} · ${item.category === 'events' ? `${item.duration} участников` : item.duration}`}</p><div className={styles.tags}>{isServiceSection(section) && item.show_home && <span>На главной</span>}{!isServiceSection(section) && !item.is_visible && <span>Скрыто</span>}</div></div>
          <div className={styles.itemActions}><button onClick={() => move(item, -1)} disabled={!index} aria-label="Переместить выше">↑</button><button onClick={() => move(item, 1)} disabled={index === items.length - 1} aria-label="Переместить ниже">↓</button><button onClick={() => setEditing(item)}>Изменить</button><button className={styles.delete} onClick={() => remove(item)}>Удалить</button></div>
        </article>)}</div>
      </section>
    </div>
  </main>;
...
function Editor({ section, initial, onCancel, onSave }) {
  ...
  async function submit(event) {
    ...
    const collection = services ? 'services' : section;
