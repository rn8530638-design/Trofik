    ...
    <div className={styles.shell}>
      ...
      <section className={styles.content}>
        ...
        <div className={styles.list} aria-busy={loading}>{loading ? <p>Загрузка…</p> : items.length === 0 ? <p className={styles.empty}>Здесь пока ничего нет. Добавьте первую карточку.</p> : items.map((item, index) => <article className={styles.item} key={item.id}>
          ...
          <div className={styles.itemText}><h2>{labelFor(item, section)}</h2><p>{section === 'reviews' ? item.text : section === 'promotions' ? `${item.discount} · ${item.expiry}` : section === 'blog' ? `${item.category} · /blog/${item.slug}` : `${item.price} · ${item.category === 'events' ? `${item.duration} участников` : item.duration}`}</p><div className={styles.tags}>{isServiceSection(section) && item.show_home && <span>На главной</span>}{section === 'blog' ? !item.is_published && <span>Черновик</span> : !isServiceSection(section) && !item.is_visible && <span>Скрыто</span>}</div></div>
