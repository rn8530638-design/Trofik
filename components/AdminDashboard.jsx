...
const sections = {
  services: { label: 'Услуги', singular: 'услугу', collection: 'services', category: 'services', description: 'Все услуги из раздела «Услуги» на странице сайта.' },
  events: { label: 'Мероприятия', singular: 'мероприятие', collection: 'services', category: 'events', description: 'Мастер-классы, праздники и обучение на странице «Услуги».' },
  price: { label: 'Прайс', singular: 'позицию', collection: 'price_items', description: 'Полный прайс-лист на странице «Услуги». Выберите группу, чтобы увидеть её позиции.' },
...
const blank = {
  ...
  price: { group_slug: PRICE_GROUPS[0].slug, name: '', description: '', duration: '', price: '', is_extra: false },
...
function Editor({ section, initial, onCancel, onSave }) {
  ...
  return <form className={styles.editor} onSubmit={submit}>
    ...
    {services && <><label>Название *<input value={form.name} onChange={(e) => set('name', e.target.value)} required /></label><label>Фото *<input type="file" accept="image/jpeg,image/png,image/webp" onChange={(e) => upload(e.target.files[0])} />{form.image_path && <img className={styles.preview} src={form.image_path} alt="Предпросмотр" />}</label><label>Описание *<textarea value={form.description} onChange={(e) => set('description', e.target.value)} required /></label><label>{form.category === 'events' ? 'Участники' : 'Длительность'}<input value={form.duration} onChange={(e) => set('duration', e.target.value)} /></label><label>Цена *<input value={form.price} onChange={(e) => set('price', e.target.value)} required /></label>{form.category === 'services' && <label className={styles.check}><input type="checkbox" checked={form.show_home} onChange={(e) => set('show_home', e.target.checked)} /> Показывать в блоке «Что мы делаем»</label>}</>}
    {section === 'price' && <><label>Группа *<select value={form.group_slug} onChange={(e) => set('group_slug', e.target.value)} required>{PRICE_GROUPS.map((group) => <option key={group.slug} value={group.slug}>{group.title}</option>)}</select></label><label>Название *<input value={form.name} onChange={(e) => set('name', e.target.value)} required /></label><label>Длительность<input value={form.duration} onChange={(e) => set('duration', e.target.value)} placeholder="Например, 1 ч 30 м" /></label><label>Цена *<input value={form.price} onChange={(e) => set('price', e.target.value)} placeholder="Например, 2 500 ₽" required /></label><label className={styles.fullWidth}>Описание<textarea value={form.description} onChange={(e) => set('description', e.target.value)} /><small>Короткая пояснительная строка под названием. Можно оставить пустым.</small></label><label className={styles.check}><input type="checkbox" checked={form.is_extra} onChange={(e) => set('is_extra', e.target.checked)} /> Дополнительная услуга — попадёт в свёрнутый блок «Дополнительно»</label></>}
